    ...
    <>
      ...
      <nav
        className={`fixed w-full z-40 transition-all duration-500 ${
          scrollPosition > 50
            ? 'modern-nav-scrolled py-3 shadow-xl'
            : 'py-5 bg-transparent'
        } ${
          !isScrollingUp && scrollPosition > 300
            ? '-translate-y-full'
            : 'translate-y-0'
        }`}
      >
          ...
          <div className="flex items-center justify-between">
            ...
            <a href="#" className="flex items-center group">
              <div className="relative">
                ...
                <div className="relative flex items-center bg-white/5 backdrop-blur-md px-3 py-2 rounded-full border border-white/10">
                  <div className="text-xl sm:text-2xl font-bold text-gradient group-hover:scale-105 transition-transform duration-300">
                    <span className="hidden xs:inline">Team</span> Blitz
                  </div>
                  <div className="animate-pulse ml-2 group-hover:animate-bounce">⚡</div>
                </div>
              </div>
            </a>
            ...
            <div className="hidden md:flex items-center space-x-1">
              <div className="bg-white/5 backdrop-blur-md rounded-full p-1.5 flex items-center">
                {navLinks.map((link) => (
                  link.href.startsWith('/') ? (
                    <Link
                      key={link.href}
                      href={link.href}
                      className={`relative px-4 py-2 rounded-full transition-all duration-300 hover:bg-white/10 group ${
                        activeSection === link.href.substring(1)
                          ? "nav-active text-white font-medium animate-glow"
                          : "text-white/80 hover:text-white"
                      }`}
                      onMouseEnter={(e) => e.currentTarget.classList.add('animate-nav-hover')}
                      onMouseLeave={(e) => e.currentTarget.classList.remove('animate-nav-hover')}
                    >
                      {link.label}
                    </Link>
                  ) : (
            ...
            <div className="md:hidden flex items-center space-x-2">
              ...
              <button
                ...
                className="p-2 rounded-full bg-gradient-to-r from-blue-600/30 to-indigo-600/30 hover:from-blue-600/50 hover:to-indigo-600/50 transition-all duration-300 border border-white/10 flex items-center justify-center"
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="p-4 border-t border-white/10 bg-gradient-to-r from-blue-900/30 to-indigo-900/30">
                <div className="flex justify-between items-center">
                  <div className="text-xs opacity-70">© 2023 Team Blitz</div>
                  <button
                    type="button"
                    onClick={() => setIsMenuOpen(false)}
